...
$lblTitlePart1.Text = "DEVICE-TWEAKER - BY adi3584_ "
...
$btnApply.Add_Click({
    ...
    foreach ($device in $deviceList) {
        ...
        elseif ($device.Category -eq "USB" -and ($normRoles -contains "Mouse" -or $normRoles -contains "Controller" -or ($normRoles -contains "Audio" -and $normRoles.Count -eq 1))) {
            ...
            if ($assignedCores -and $assignedCores.Count -gt 0) {
                if ($normRoles -contains "Audio" -and $normRoles.Count -eq 1) {
            ...
            if ($device.Category -eq "Network") {
                if ($assignedCores) { $weakOccupiedCores += $assignedCores }
            }

            if ($normRoles -contains "Keyboard" -and $normRoles -contains "Audio") {
                if ($assignedCores) { $weakOccupiedCores += $assignedCores }
            }

            if ($device.Category -eq "USB" -and ($normRoles -contains "Keyboard") -and ($normRoles.Count -eq 1)) {
                if ($assignedCores) { $weakOccupiedCores += $assignedCores }
            }

            if ($device.Category -eq "PCI" -and $device.Role -eq "Audio") {
                if (-not $assignedCores -or $assignedCores.Count -eq 0) {
                    $ctrls = $deviceControls[$device]
                    $assignedCores = @()

                    try {
                        $currAff = Get-CurrentAffinity $device.RegistryPath $false
                    } catch { $currAff = "0x0" }

                    if ($currAff -and $currAff -match '^0x[0-9A-Fa-f]+$') {
                        $maskText = ($currAff -replace '0x','').Trim()
                        if ($maskText -and ([int]::TryParse($maskText, [System.Globalization.NumberStyles]::HexNumber, $null, [ref]$null))) {
                            $maskInt = [Convert]::ToInt64($maskText, 16)
                            $binary = [Convert]::ToString($maskInt, 2).PadLeft([Environment]::ProcessorCount, '0')
                            for ($i = 0; $i -lt $binary.Length; $i++) {
                                if ($binary[$i] -eq '1') { $assignedCores += ($binary.Length - $i - 1) }
                            }
                        }
                    }
